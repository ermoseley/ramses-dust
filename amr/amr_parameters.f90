module amr_parameters
 ...
 ! classic tracer                                                                                                            
  ...
  real(dp)::stopping_rate=-1.0       ! When greater than or equal to zero, overrides t_stop for constant_t_stop==.true. Allow\
s for zero drag.                                                                                                              
  real(dp)::grain_size=1.0           ! Grain size parameter rho_d^i r_d/(rho_g l_0).                                          
  logical::constant_t_stop=.false.    ! Dictates whether stopping time is constant t_stop, or uses grain_size, gas density, v\
elocity, etc.                                                                                                                 
  ...
  logical :: supersonic_drag=.true.   ! if true, Epstein drag is used. If false, drag depends only on density and the sound s\
peed.                                                                                                                         
...
e ndust evenly.                                                                                                               
 ...
 with sigma=ddex/4                                                                                                            
  logical :: astrodust2 = .false. ! Creates an astrodust distribution (Hensley & Draine '23) that extends from 0.5*peak to 2*\
peak (peak (grain_size) = 0.23 microns)                                                                                       
  logical :: astrodust4 = .false. ! Creates an astrodust distribution (Hensley & Draine '23) that extends from 0.25*peak to 4\
*peak (peak (grain_size) = 0.23 microns)                                                                                      
  ...
  real(dp):: cdex=0.0                 ! Determines how many decades the cr spectrum spans in energy (momentum?) Not implement\
ed.                                                                                                                           
  real(dp):: cr_slope=0.0         ! Determines how the grain charge scales with grain size (power law option). Should be nega\
tive. Not implemented.                                                                                                        
  logical:: cr_emf_feedback=.false. ! Not yet implemented. Determines whether or not cosmic rays impact the induction equatio\
n.        
real(dp):: cr_c_fraction=1.0      ! factor for RSOL formulation.                                                            
real(dp),dimension(1:3):: cr_boost=0 ! Velocity by which to boost the CR population. Must be << c, given current implementa\
tion.                                                                                                                         
